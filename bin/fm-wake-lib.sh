...
# Its identity is the project's resolved origin, so separate clones of one origin
# share a single lock; an origin-less local-only project falls back to its own
# worktree top instead of failing to resolve.
#
# Each home allocates from its own pool now (bin/fm-treehouse-lib.sh), so the
# lock no longer guards a shared pool. It still guards a shared CLONE: two homes
# pointed at one checkout add and remove linked worktrees in the same repository
# metadata, and that is exactly the sharing this project keeps supporting.
